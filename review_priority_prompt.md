...
**每个优先级区域必须包含关键的代码示例**，以帮助 reviewer 快速定位和理解：

1. **code_snippet 字段**：在每个 priority_areas 项中添加 `code_snippet` 对象，包含以下信息：
   - `file_path`: 文件路径
   - `diff`: 统一 diff 格式的代码变更，展示需要重点关注的部分
     - 使用 `+` 标记新增的代码行
     - 使用 `-` 标记删除的代码行
     - 包含必要的上下文行（前后 2-3 行）
   - `lines_added`: 新增行数
...
3. **重要提示**：
   - 代码片段应展示需要重点 review 的部分，长度控制在 10-25 行
   - 优先展示高风险和复杂的代码变更
   - diff 格式必须准确反映实际变更
