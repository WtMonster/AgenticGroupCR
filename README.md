...
以下是计划中的功能改进，按优先级从高到低排列：

### P0 - 高优先级

- [ ] **运行状态实时输出**：支持脚本运行过程中输出工具调用情况，避免用户无法区分程序 hang 住和 review 耗时长的情况

### P1 - 中高优先级

- [ ] **灵活的项目指定方式**：从 appid 的应用指定方式，改为更灵活的方式：
  - 默认支持传入项目绝对路径
  - 支持传入 git 链接（自动 clone）
  - 支持用户自定义扩展，用户可以自定义查找项目的方式（如通过 appid、项目名等）

### P2 - 中优先级

- [ ] **优化报告展示**：优化报告展示逻辑，支持相关代码段的展示，让 review 结果更直观

### P3 - 低优先级

- [ ] **支持 Cursor CLI**：集成 Cursor CLI 作为新的 AI 引擎
- [ ] **支持 Gemini CLI**：集成 Google Gemini CLI 作为新的 AI 引擎
